...
export function note(message: string, title?: string): void {
  ...
  if (title) {
    output.write(`${gray(S_BAR)}  ${getTheme().success(bar)}\n`);
    output.write(`${gray(S_BAR)}  ${getTheme().success(bold(title))}\n`);
  } else {
...
export const log = {
  info(message: string): void {
    writeLine(getTheme().info("\u2139"), message, "info");
  },

  success(message: string): void {
    writeLine(getTheme().success(S_STEP_SUBMIT), message, "success");
  },

  warn(message: string): void {
    writeLine(getTheme().warning(S_STEP_ERROR), message, "warn");
  },

  error(message: string): void {
    writeLine(getTheme().error(S_STEP_ERROR), message, "error");
  },
